class RetiredFeeCuttingsController < ApplicationController
  ...
  def get_json
    ...
    if(params[:search_name] && params[:search_name].to_s!='')
      @retired_fee_cuttings = RetiredFeeCutting.paginate(:order =>"id DESC", :conditions => ["name like ?","%#{params[:search_name]}%"],:per_page=>pagesize,:page => params[:page] || 1)
      count = RetiredFeeCutting.count(:conditions =>["name like ?","%#{params[:search_name]}%"])
    else
