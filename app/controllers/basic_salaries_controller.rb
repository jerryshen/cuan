class BasicSalariesController < ApplicationController
  ...
  def get_json
    ...
    if(!params[:search_name].blank? && params[:search_department_id].blank?)
      ...
      @basic_salaries = BasicSalary.paginate(:order =>"id DESC", :conditions => ["user_id =?",user_id],:per_page=>pagesize, :page => params[:page] || 1)
      ...
      @basic_salaries = BasicSalary.paginate(:order => "id DESC", :joins =>"INNER JOIN users p ON basic_salaries.user_id=p.id" , :conditions =>["p.department_id =? and p.name like ?",params[:search_department_id],"%#{params[:search_name]}%"],:per_page=>pagesize, :page => params[:page] || 1 )
      count = @basic_salaries.length
    else
