class DataExport
  ...
  def self.get_salary_data(year,month)
    ...
    sql = "SELECT aa.user_id,(#{reg}) salary  FROM basic_salary_records as aa INNER JOIN fee_cutting_records as bb ON bb.user_id = aa.user_id WHERE (aa.year = #{year} and bb.year = #{year} and aa.month = #{month} and bb.month = #{month}) GROUP BY aa.user_id"
